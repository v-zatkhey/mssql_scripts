...
SET @myID_0 = '-'
select	@Kod = I.Код, 
		@Decision_2 = I.[Решение_по_поставке], @Decision_old_2 = D.[Решение_по_поставке],
...
IF (@Decision_2 <> @Decision_old_2) and @Decision_2 = 1
BEGIN
      insert into MaterialInfo(MatID,CustID,CustMatName,DateInsert,Note)
		select   
			...
			,c.CustID
			,m.MatName 
			,cast(p.Дата_пост as date) as LotDate
			,'auto_tr_post'
		from tblПоставки p
			inner join dbo.Customers c on c.CustName = p.Код_поставщика
			inner join Materials m on m.MatName = p.ID
		where [Код] = @Kod
			and not exists(select top 10 * from MaterialInfo where MatID = m.MatID and CustID = c.CustID)
END
